...
client.on("messageCreate", async (message) => {
  if (message.content === "!ticketpanel") {
    ...
    const row = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder()
        .setCustomId("staff")
        .setLabel("Staff Sollicitatie")
        .setStyle(ButtonStyle.Success),

      new ButtonBuilder()
        .setCustomId("dev")
        .setLabel("Dev Sollicitatie")
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (onderwerpMap[interaction.customId]) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Welkom ${interaction.user}, leg hier je ${onderwerp.toLowerCase()} uit.`
      )
      .setColor(0x5865f2);
  ...
  if (interaction.customId === "delete") {
    interaction.reply({
      content: "Ticket wordt verwijderd 🗑️",
      ephemeral: true
    });
  ...
  if (interaction.customId === "transcript") {
    ...
    interaction.user
      .send(`📄 Transcript van ${interaction.channel.name}:\n\n${transcript}`)
      .catch(() => {
        interaction.reply({
          content: "Kon transcript niet verzenden (DMs uitgeschakeld)",
          ephemeral: true
        });
      });

    interaction.reply({
      content: "Transcript verzonden 📄",
      ephemeral: true
    });
  }
});
